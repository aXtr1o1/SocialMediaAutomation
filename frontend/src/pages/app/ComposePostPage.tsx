...
export function ComposePostPage() {
  ...
  const selection = isComposeState(location.state) ? location.state : null
  ...
  async function handleGenerate() {
    ...
    try {
      const articleId = selection.article.article_id || selection.article.id
      const result = await generatePosts(articleId, platformsFromChoice(choice))
      ...
      setError(caught instanceof Error ? caught.message : 'Could not generate the post')
  ...
  if (!selection) {
    return (
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-lg p-lg lg:p-xl">
        <h1 className="font-headline-md text-headline-md text-on-surface">Compose Post</h1>
    ...
    <div className="relative flex h-[calc(100vh-64px)] w-full flex-col items-center justify-center overflow-hidden bg-surface">
      ...
      <div className="relative z-10 mx-md flex w-full max-w-[480px] flex-col gap-lg rounded-xl bg-surface-container-lowest p-xl shadow-xl">
        ...
        <div className="grid grid-cols-3 gap-sm">
          <PlatformChoiceCard
            label="LinkedIn"
            selected={choice === 'linkedin'}
            disabled={isGenerating}
            onSelect={() => setChoice('linkedin')}
          >
          ...
          <PlatformChoiceCard
            label="Bluesky"
            selected={choice === 'bluesky'}
            disabled={isGenerating}
            onSelect={() => setChoice('bluesky')}
          >
            <div className="mb-sm flex h-12 w-12 items-center justify-center rounded-full bg-surface-container transition-colors group-hover:bg-primary/5">
        ...
        {error ? <p className="text-center font-body-md text-body-md text-error">{error}</p> : null}
        ...
        >
          <MaterialIcon name={isGenerating ? 'progress_activity' : 'auto_awesome'} className={`text-[18px] ${isGenerating ? 'animate-spin' : ''}`} />
