...
function PasswordToggle({
  ...
  return (
    <button
      ...
      aria-label={label}
    >
      <MaterialIcon name={isVisible ? 'visibility_off' : 'visibility'} className="text-[20px]" />
    </button>
  )
}
...
export function ProfilePage() {
  ...
  async function handleAvatarSelected(file: File | null) {
    ...
    try {
      ...
      setAvatarError(caught instanceof Error ? caught.message : 'Could not upload photo')
      setPreviewUrl((previous) => {
        if (previous) {
          URL.revokeObjectURL(previous)
        }
  ...
  async function handleRemoveAvatar() {
    ...
    try {
      ...
      setAvatarError(caught instanceof Error ? caught.message : 'Could not remove photo')
    } finally {
      setIsUploadingAvatar(false)
    }
  }
  ...
  async function handlePasswordSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (caught instanceof ApiError) {
        ...
        setPasswordError(caught instanceof Error ? caught.message : 'Could not update password')
      }
    } finally {
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-xl px-lg py-xl">
      ...
      >
        <div className="mb-lg flex items-center gap-md border-b border-surface-variant pb-lg">
          <div className="relative shrink-0">
            ...
            >
              <MaterialIcon
                name={isUploadingAvatar ? 'progress_activity' : 'photo_camera'}
                className={`text-[16px] ${isUploadingAvatar ? 'animate-spin' : ''}`}
              />
            </button>
            <input
              ref={fileInputRef}
          ...
          <div className="min-w-0 flex-1">
            ...
            {avatarError ? <p className="mt-1 text-[13px] text-error">{avatarError}</p> : null}
            {avatarNotice ? <p className="mt-1 text-[13px] text-primary">{avatarNotice}</p> : null}
          </div>
        </div>
      ...
      >
          ...
          <ProfileField
            ...
            rightSlot={
              <PasswordToggle
                ...
                label={currentVisibility.isVisible ? 'Hide password' : 'Show password'}
              />
            }
          />
            ...
            <ProfileField
              ...
              rightSlot={
                <PasswordToggle
                  ...
                  label={newVisibility.isVisible ? 'Hide password' : 'Show password'}
                />
              }
            />
          ...
          <ProfileField
            ...
            rightSlot={
              <PasswordToggle
                ...
                label={confirmVisibility.isVisible ? 'Hide password' : 'Show password'}
              />
            }
          />
        </div>
        ...
        <div className="mt-xl flex flex-col items-end gap-sm border-t border-surface-variant pt-lg">
          {passwordError ? <p className="w-full text-right text-[13px] text-error">{passwordError}</p> : null}
          {passwordNotice ? (
            <p className="w-full text-right text-[13px] text-primary">{passwordNotice}</p>
          ) : null}
          <button
            type="submit"
